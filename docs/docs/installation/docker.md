...
docker run -it --rm -e SILICONFLOW_API_KEY=${SILICONFLOW_API_KEY} \
 -p 6666:6666 --name dbgpt eosphorosai/dbgpt-openai
```

Please replace `${SILICONFLOW_API_KEY}` with your own API key.


Then you can visit [http://localhost:6666](http://localhost:6666) in the browser.
...
Once the container is running, you can visit [http://localhost:6666](http://localhost:6666) in your browser to access the application.
