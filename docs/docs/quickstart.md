...
To run DB-GPT with OpenAI proxy, you must provide the OpenAI API key in the `configs/derisk-proxy-deepseek.toml` configuration file or privide it in the environment variable with key `OPENAI_API_KEY`.
...
To run DB-GPT with the local model. You can modify the `configs/derisk-local-glm.toml` configuration file to specify the model path and other parameters.
...
To run DB-GPT with the local model. You can modify the `configs/derisk-local-vllm.toml` configuration file to specify the model path and other parameters.
...
To run DeRisk with the local model. You can modify the `configs/derisk-local-sglang.toml` configuration file to specify the model path and other parameters.
...
```bash
uv run derisk start webserver --config configs/derisk-local-sglang.toml
```
...
Open your browser and visit [`http://localhost:6666`](http://localhost:6666)
...
cd web && npm install
cp .env.template .env
// Set API_BASE_URL to your DeRisk server address, usually http://localhost:6666
npm run dev
```
