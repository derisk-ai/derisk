...
@dataclass
class BailingRerankEmbeddingsParameters(OpenAPIRerankerDeployModelParameters):
    """bailing Rerank Embeddings Parameters."""

    provider: str = "proxy/bailing"

    api_url: str = field(
        default="https://antchat.alipay.com/v1/rerank",
        metadata={
            "help": _("The URL of the rerank API."),
        },
    )
    api_key: Optional[str] = field(
        default="${env:BAILING_API_KEY}",
        metadata={
            "help": _("The API key for the rerank API."),
        },
    )
    backend: str = field(
        default="bge_reranker_v2_m3",
        metadata={
...
class BailingRerankEmbeddings(OpenAPIRerankEmbeddings):
    """Bailing Rerank Model.

    See `Bailing API
    <https://docs.bailing.cn/api-reference/rerank/create-rerank>`_ for more details.
    """

    def __init__(self, **kwargs: Any):
        """Initialize the BailingRerankEmbeddings."""
        # If the API key is not provided, try to get it from the environment
        if "api_key" not in kwargs:
            kwargs["api_key"] = os.getenv("BAILING_API_KEY")

        if "api_url" not in kwargs:
            env_api_url = os.getenv("BAILING_API_BASE")
            if env_api_url:
                env_api_url = env_api_url.rstrip("/")
                kwargs["api_url"] = env_api_url + "/rerank"
            else:
                kwargs["api_url"] = "https://antchat.alipay.com/v1/rerank"

        if "model_name" not in kwargs:
            kwargs["model_name"] = "bge_reranker_v2_m3"

        super().__init__(**kwargs)

    @classmethod
    def param_class(cls) -> Type[BailingRerankEmbeddingsParameters]:
        """Get the parameter class."""
        return BailingRerankEmbeddingsParameters
    ...
    def predict(self, query: str, candidates: List[str]) -> List[float]:
        ...
        if self.pass_trace_id and current_span_id:
            # Set the trace ID if available
            headers[DERISK_TRACER_SPAN_ID] = current_span_id
        data = {"model": self.model_name, "query": query, "texts": candidates}
        response = self.session.post(  # type: ignore
            self.api_url, json=data, timeout=self.timeout, headers=headers
        )
        response.raise_for_status()
...
register_embedding_adapter(
    BailingRerankEmbeddings, supported_models=RERANKER_COMMON_HF_MODELS
)
